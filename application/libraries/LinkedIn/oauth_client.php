...
*/
	...
	function SendAPIRequest($url, $method, $parameters, $oauth, $options, &$response)
	{
		...
		if (
			strlen($error = $http->SendRequest($arguments))
		...
		if (
			...
			if (
				isset($options['FailOnAccessError'])
	...
	function ProcessToken1($oauth, &$access_token)
	{
		...
		if (
			isset($response['oauth_expires_in'])
			...
			if (
				strval($expires) !== strval(intval($expires))
				|| $expires <= 0
			)
	...
	function ProcessToken2($code, $refresh)
	{
		...
		$this->access_token = '';
		if (
			isset($response[$name = 'access_token'])
		...
		if (
			isset($response[$name = 'refresh_token'])
